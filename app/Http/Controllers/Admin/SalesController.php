...
class SalesController extends JoshController
{
    ...
    public function edit($id)
    {
        ...
        return view('admin.salesReferral.edit', compact('sales_referral','company_types','customer_industries'));
    }
    ...
    public function salesReferralAssignToClient(Request $request)
    {
        ...
        if ($sales_referralstatuscheck === null) {
            ...
            if($input['lead_status'] === '2'){
                ...
                $customer_registercheck = User::where('referral_id', '=', $input['sales_referral_id'])->where('email', '=', $input['email'])->first();
                if(!empty($customer_registercheck))
                {
                    $applyas = '2';
                    $otp = rand(1000,9999);
