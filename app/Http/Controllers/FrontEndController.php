...
class FrontEndController extends JoshController
{
    ...
    public function postLogin(Request $request)
    {
        ...
        try {
            ...
            if ($user=  Sentinel::authenticate($request->only('email', 'password'), $request->get('remember-me', 0))) {
                ...
                return Redirect::route("profile")->with('success', trans('auth/message.login.success'));
            } else {
    ...
    public function postRegister(UserRequest $request)
    {
        ...
        try {
            ...
            return Redirect::route("profile")->with('success', trans('auth/message.signup.success'));
    ...
    public function getActivate($userId, $activationCode)
    {
        ...
        if (Activation::complete($user, $activationCode)) {
            ...
            return Redirect::route('account/login')->with('success', trans('auth/message.activate.success'));
        } else {
            // Activation not found or not completed.
            $error = trans('auth/message.activate.error');
            return Redirect::route('account/login')->with('error', $error);
        }
    }
    ...
    public function postForgotPasswordConfirm(PasswordResetRequest $request, $userId, $passwordResetCode = null)
    {
        ...
        if (!$reminder = Reminder::complete($user, $passwordResetCode, $request->get('password'))) {
            // Ooops.. something went wrong
            return Redirect::route('account/login')->with('error', trans('auth/message.forgot-password-confirm.error'));
        }

        // Password successfully reseted
        return Redirect::route('account/login')->with('success', trans('auth/message.forgot-password-confirm.success'));
    }
